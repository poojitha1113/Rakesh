...
public class DestroyOrganizationTest extends base.prepare.Is4Org{
...
//	  private String buildVersion;
	...
	public void deactiveOrg(String username, String password, String organizationID) {
		...
		org4spogServer.destroyOrganizationWithErrorCheck(organizationID, 403, "00900003", test);
		
		spogServer.userLogin(csr_readonly_email, csr_readonly_password);
		org4spogServer.setToken(spogServer.getJWTToken());
		org4spogServer.destroyOrganizationWithErrorCheck(organizationID, 403, "00100101", test);
		
		
		spogServer.userLogin(csrAdmin, csrPwd);
		org4spogServer.setToken(spogServer.getJWTToken());
		
		org4spogServer.destroyOrganization(organizationID, test);
		spogServer.userLoginWithErrorCheck(username, password, 401, "00200022", test);
		spogServer.GetOrganizationInfobyIDWithExpectedStatusCode(organizationID, 404, "0030000A", test);
		org4spogServer.destroyOrganizationWithErrorCheck(UUID.randomUUID().toString(), 404, "0030000A", test);
		org4spogServer.destroyOrganizationWithErrorCheck("uuid", 400, "40000005", test);
		org4spogServer.setToken("");
		org4spogServer.destroyOrganizationWithErrorCheck(organizationID, 401, "00900006", test);
	}
