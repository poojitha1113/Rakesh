...
public class GetMailsForAlertsTest extends base.prepare.Is4Org {
	...
	public void getEmailsForAlerts(String org_type, String org_id, String token, String recipient, String alertName,
		...
		ArrayList<HashMap> exp_data = new ArrayList<HashMap>();
		Map<String, Object> composedData = new HashMap<>();
		String alert_email_id = null;

		for (String item : type) {
			composedData = jp.composeAlertsInfo(item, alertName, org_id, recipient, Report_for_type, extra_id);
			alert_email_id = spogAlertsServer.submitEmailsForAlertswithcheck(token, composedData, user_id, user_email,
					allowed_actions, expectedStatusCode, expectedErrorMessage, test);
			exp_data.add((HashMap) composedData);

			if (org_type.contains("monitor"))
				spogAlertsServer.getEmailsForAlertswithcheck(monitor_token, exp_data, user_id, user_email,
						allowed_actions, SpogConstants.SUCCESS_GET_PUT_DELETE, SpogMessageCode.SUCCESS_GET_PUT_DEL,
						test);
			else
				spogAlertsServer.getEmailsForAlertswithcheck(token, exp_data, user_id, user_email, allowed_actions,
	...
	public final Object[][] alertsInfo_invalid() {
		return new Object[][] {
				{ "invalid token", ti.direct_org1_id, type.get(0), "asdf", "test.direct@gmail.com",
						ti.direct_org1_user1_id, ti.direct_org1_user1_token, SpogConstants.NOT_LOGGED_IN,
	...
	public void getEmailsForAlerts_invalid(String testcase, String org_id, String alert_type, String token,
		...
		spogAlertsServer.getEmailsForAlertswithcheck(token, new ArrayList<HashMap>(), user_id, user_email,
				allowed_actions, expectedStatusCode, expectedErrorMessage, test);
	}
