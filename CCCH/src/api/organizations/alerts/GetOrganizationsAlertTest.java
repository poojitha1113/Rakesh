...
public class GetOrganizationsAlertTest extends base.prepare.Is4Org {
	...
	// volume
	@DataProvider(name = "getOrganizationsAlerts")
	public final Object[][] getOrganizationsAlerts() {
		return new Object[][] {
				...
				{ ti.normal_msp1_suborg1_user1_email, ti.common_password, ti.normal_msp1_suborg1_user1_token,
						"normal_sub", ti.normal_msp1_suborg1_id, ti.normal_msp1_suborg1_user1_id, msp_cloud_account_id,
						...
						new String[] { LogSeverityType.information.name(), LogSeverityType.warning.name(),
								LogSeverityType.error.name() },
						test_log_Message_1, new String[] { "node", "10.57.63.2" }, true, System.currentTimeMillis(),
						"arcserve@spog", "normal_sub", ti.normal_msp1_suborg1_name },

				{ ti.root_msp1_suborg1_user1_email, ti.common_password, ti.root_msp1_suborg1_user1_token, "root_sub",
						...
						new String[] { LogSeverityType.information.name(), LogSeverityType.warning.name(),
								LogSeverityType.error.name() },
						test_log_Message_1, new String[] { "node", "10.57.63.2" }, true, System.currentTimeMillis(),
						"arcserve@spog", "root_sub", ti.root_msp1_suborg1_name },
				{ ti.msp1_submsp1_suborg1_user1_email, ti.common_password, ti.msp1_submsp1_suborg1_user1_token,
						"submsp_sub", ti.msp1_submsp1_sub_org1_id, ti.msp1_submsp1_suborg1_user1_id, root_cloud_id,
						...
						new String[] { LogSeverityType.information.name(), LogSeverityType.warning.name(),
								LogSeverityType.error.name() },
						test_log_Message_1, new String[] { "node", "10.57.63.2" }, true, System.currentTimeMillis(),
						"arcserve@spog", "submsp_sub", ti.msp1_submsp1_sub_org1_name }, };
	}

	@Test(dataProvider = "getOrganizationsAlerts", dependsOnMethods = "deleteResources")
	public void getOrganizationsAlert_200(String userName, String password, String adminToken, String organization_name,
		...
		spogAlertsServer.deleteOrganizationAlerts_Acknowledgeall(adminToken, SpogConstants.SUCCESS_GET_PUT_DELETE, null,
				test);
		...
		for (int i = 0; i < 1; i++) {
			...
			job_id = gatewayServer.postJobWithCheck(start_time_ts, endTimeTS, organization_id, source_id, source_id,
					rps_id, destination_id, policy_id, job_type[i], job_method[i], job_status[i], site_Token, test);

			last_job = spogDestinationServer.composeLastJob(start_time_ts, endTimeTS, percent_complete, job_status[i],
			...
			if (i == 1) {
				alert_data.add("critical");
				type[i] = "backup_errors";
			} else {
				alert_data.add("critical");
				type[i] = "recovery_errors";
			}
			String token = null;
			if (orgType.contains("direct"))
				token = ti.direct_org1_monitor_user1_token;
			else if (orgType.contains("normal_sub"))
				token = ti.normal_msp1_suborg1_monitor_user1_token;
			else if (orgType.contains("root_sub"))
			...
			String alert_id = alert_id_1.get(0);

			test.log(LogStatus.INFO, "check the organization Alerts " + org_name);
			spogAlertsServer.checkGetAlertsData(response, organization_id, org_name, orgType, job_id, job_severity[i],
					alert_details, alert_id_1, SpogConstants.SUCCESS_GET_PUT_DELETE,
					SpogMessageCode.SUCCESS_GET_PUT_DEL, test);

			test.log(LogStatus.INFO, "get the organization Alerts with monitor token ");
			response = spogAlertsServer.getOrganizationAlerts(token, organization_id,
					SpogConstants.SUCCESS_GET_PUT_DELETE, test);

			alert_id_1 = response.then().extract().path("data.alert_id");
			alert_id = alert_id_1.get(0);
			...
			test.log(LogStatus.INFO, "delete the Creating organization Alerts ");
			response = spogAlertsServer.deleteOrganizationAlertsByAlertId(adminToken, organization_id, alert_id,
					SpogConstants.SUCCESS_GET_PUT_DELETE, test);
	...
	@DataProvider(name = "getOrganizationsAlerts_org_enroll")
	public final Object[][] getOrganizationsAlerts_org_enroll() {
		return new Object[][] { { "direct", ti.direct_org2_id, ti.direct_org2_name },
				{ "msp", ti.normal_msp_org2_id, ti.normal_msp_org2_name },
				{ "rootmsp", ti.root_msp_org2_id, ti.root_msp_org2_name }, };
	}

	@Test(dataProvider = "getOrganizationsAlerts_org_enroll")
	public void getOrganizationsAlert_OrganizationEnrollment_200(String org_type, String org_id,
		...
		test.log(LogStatus.INFO, "get the organization Alerts ");
		Response response = spogAlertsServer.getOrganizationAlerts(ti.csr_token, org_id,
				SpogConstants.SUCCESS_GET_PUT_DELETE, test);

		test.log(LogStatus.INFO, "check the organization Alerts ");
		spogAlertsServer.checkGetAlertsData(response, org_id, organization_name, org_type, null, "information", null,
				null, SpogConstants.SUCCESS_GET_PUT_DELETE, SpogMessageCode.SUCCESS_GET_PUT_DEL, test);

	}

	@DataProvider(name = "getOrganizationsAlerts_403")
	public final Object[][] getOrganizationsAlerts_403() {
		return new Object[][] {
	...
	@DataProvider(name = "getOrganizationsAlerts_invalid")
	public final Object[][] getOrganizationsAlerts_invalid() {
	...
	@Test(dataProvider = "getOrganizationsAlerts_invalid")
	public void getOrganizationsAlerts_invalid_401(String userName, String password, String adminToken,
		...
		test.log(LogStatus.INFO, "get organization Alerts with invalidtoken");
		Response response = spogAlertsServer.getOrganizationAlerts("abc", organization_id, SpogConstants.NOT_LOGGED_IN,
				test);

		test.log(LogStatus.INFO, "check response of organization Alerts ");
		...
		test.log(LogStatus.INFO, "get organization Alerts with missed token");
		response = spogAlertsServer.getOrganizationAlerts("", organization_id, SpogConstants.NOT_LOGGED_IN, test);
	...
	@Test(dataProvider = "getOrganizationsAlerts_invalid")
	public void getOrganizationsAlert_invalid_400(String userName, String password, String adminToken,
			String organization_name, String organization_id, String create_user_id, String site_id, Long start_time_ts,
			Long endTimeTS, String site_Token, String status, String job_seq, Double percent_complete,
			String protected_data_size, String raw_data_size, String sync_read_size, String ntfs_volume_size,
			String virtual_disk_provision_size, String dedupe_savings, String logSourceType[], String LogSeverity[],
		...
		int i = 0;
		test.log(LogStatus.INFO, "get organization Alerts with token, random org id");
		Response response = spogAlertsServer.getOrganizationAlerts(adminToken, "abc",
	...
	@Test(dataProvider = "getOrganizationsAlerts_403")
	public void getOrganizationsAlerts_403(String testcase, String organization_id, String orgType, String org_name,
		...
		int i = 0;
		test.log(LogStatus.INFO, "get organization Alerts with other org token" + orgType);
		Response response = spogAlertsServer.getOrganizationAlerts(otherOrgToken, organization_id,
