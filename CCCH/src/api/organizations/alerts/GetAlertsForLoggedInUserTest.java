...
public class GetAlertsForLoggedInUserTest extends base.prepare.Is4Org {
	private SPOGServer spogServer;
	private SPOGAlertsServer spogAlertServer;
	...
	public void beforeClass(String baseURI, String port, String logFolder, String runningMachine, String buildVersion) {
		...
		spogAlertServer = new SPOGAlertsServer(baseURI, port);
		...
		return new Object[][] {
				...
				{ ti.msp1_submsp1_suborg1_user1_email, ti.common_password, ti.msp1_submsp1_suborg1_user1_token,
						...
						new String[] { LogSeverityType.information.name(), LogSeverityType.warning.name(),
								LogSeverityType.error.name() },
						test_log_Message_1, new String[] { "node", "10.57.63.2" }, true, System.currentTimeMillis(),
						"arcserve@spog", "sumsp_sub", ti.msp1_submsp1_sub_org1_name }, };
	}

	@Test(dataProvider = "getOrganizationsAlerts", dependsOnMethods = "deleteResources")
	public void getAlertsForLoggenInuser_200(String userName, String password, String adminToken,
		...
		response = spogAlertServer.getOrganizationAlerts(adminToken, organization_id,
		...
		for (int j = 0; j < alert_id_1.size(); j++) {
			...
			spogAlertServer.deleteOrganizationAlertsByAlertId(adminToken, organization_id, alert_id_1.get(j),
					SpogConstants.SUCCESS_GET_PUT_DELETE, test);
		}
		...
		for (int i = 0; i < 1; i++) {
			...
			spogAlertServer.setToken(adminToken);
			...
			test.log(LogStatus.INFO, "get the alerts for the logged in user");
			response = spogAlertServer.getAlertsForLoggedInUser(adminToken, "", SpogConstants.SUCCESS_GET_PUT_DELETE,
					test);

			// get alerts for logged in user with csrreadonly valid token

			String additionalurl = "organization_id=" + organization_id;
			response = spogAlertServer.getAlertsForLoggedInUser(ti.csr_readonly_token, additionalurl,
					SpogConstants.SUCCESS_GET_PUT_DELETE, test);

			alert_id_1 = response.then().extract().path("data.alert_id");
			String alert_id = alert_id_1.get(0);

			test.log(LogStatus.INFO, "check the  Alerts " + org_name);
			spogAlertServer.checkGetAlertsData(response, organization_id, org_name, orgType, job_id, job_severity[i],
					alert_details, alert_id_1, SpogConstants.SUCCESS_GET_PUT_DELETE,
					SpogMessageCode.SUCCESS_GET_PUT_DEL, test);

			test.log(LogStatus.INFO, "get alerts for logged in user using monitor token");
			response = spogAlertServer.getAlertsForLoggedInUser(token, additionalurl,
					SpogConstants.SUCCESS_GET_PUT_DELETE, test);
			...
			response = spogAlertServer.deleteOrganizationAlertsByAlertId(adminToken, organization_id, alert_id,
					SpogConstants.SUCCESS_GET_PUT_DELETE, test);
	...
	public final Object[][] getOrganizationsAlerts_invalid() {
		return new Object[][] { { "direct", ti.direct_org1_id, ti.direct_org1_name }, };
	}

	// get the user defined job columns using csr/valid token
	@Test(dataProvider = "getOrganizationsAlerts_invalid")
	public void getAlertsForLoggedInUser_Invalid_401(String org_type, String org_id, String org_name) {
		...
		Response response = spogAlertServer.getAlertsForLoggedInUser("abc", "", SpogConstants.NOT_LOGGED_IN, test);

		test.log(LogStatus.INFO, "check response of organization Alerts ");
		spogAlertServer.checkAlertData(response, alert_id, org_id, org_name, job_id, type[i], job_severity[i],
		...
		response = spogAlertServer.getAlertsForLoggedInUser("", "", SpogConstants.NOT_LOGGED_IN, test);
