...
/** This is to test the Cloud hybrid destination destroy (deletion)
 * 
 *  On calling the api DELETE: /cloudhybridstores/{id}/destroy Cloud hybrid store will be removed completely
 *    
 * Prerequisites:
 ...
 * @sprint 28
 */
public class DestroyCloudHybridStoreByIdTest extends base.prepare.Is4Org {
	...
	public Object[][] destroyCHSInfo(){
		return new Object[][] {
			...
			{"Destroy cloud hybrid store with msp user token",
			...
			{"Destroy cloud hybrid store with sub org user token",
			...
			{"Destroy cloud hybrid store with msp account admin token",
			...
			{"Destroy cloud hybrid store with sub msp user token",
							ti.root_msp1_suborg1_user1_token, hybridStore_id, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Destroy cloud hybrid store with sub msp sub org user token",
			...
			{"Destroy cloud hybrid store with sub msp account admin token",
	...
	public void destroyCloudHybridStore(String caseType,
		...
		test.log(LogStatus.INFO, caseType);
		spogcloudRPSServer.destroyCloudHybridStoreByIdWithCheck(token, hybrdstore_id, expectedStatusCode, expectedErrorMessage, test);
	}
