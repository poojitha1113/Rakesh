...
/** This is to stop the Cloud hybrid destination that is in running state
 * 
 * On calling the api POST: /cloudhybridstores/{id}/stop Cloud hybrid store will be stopped if it is in running state
 *    
 * Prerequisites:
 ...
 * 4. Cloud hybrid store that is in running state
 *
 ...
 * @sprint 28
 */
public class StopCloudHybridStoreTest extends base.prepare.Is4Org {
	...
	public Object[][] stopCHSInfo(){
		return new Object[][] {
			...
			{"Stop cloud hybrid store with msp user token",
			...
			{"Stop cloud hybrid store with sub org user token",
			...
			{"Stop cloud hybrid store with msp account admin token",
			...
			{"Stop cloud hybrid store with sub msp user token",
			...
			{"Stop cloud hybrid store with sub msp sub org user token",
			...
			{"Stop cloud hybrid store with sub msp account admin token",
			...
			{"Stop cloud hybrid store with csr read only user token",
						ti.csr_readonly_token, hybridStore_id, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			
			//404
			{"Stop cloud hybrid store with hybridstore id that does not exist",
						ti.csr_token, UUID.randomUUID().toString(), SpogConstants.RESOURCE_NOT_EXIST, SpogMessageCode.DESTINATION_NOT_FOUND},
			
			//200
			{"Stop cloud hybrid store with valid hybridstore id",
						ti.csr_token, hybridStore_id, SpogConstants.SUCCESS_GET_PUT_DELETE, null},
			
			//405
			{"Stop cloud hybrid store that is already in stopped state",
							ti.csr_token, hybridStore_id, SpogConstants.REQUEST_CONFLICT, SpogMessageCode.DATASTORE_IS_ALREADY_STOPPED},
		};
	}
	
	@Test(dataProvider="stopCHSInfo",dependsOnMethods= {"createCloudHybridStores"})
	public void destroyCloudHybridStore(String caseType,
										String token, 
		...
		test.log(LogStatus.INFO, caseType);
		spogcloudRPSServer.stopCloudHybridStoreByIdWithCheck(token, hybrdstore_id, expectedStatusCode, expectedErrorMessage, test);
		
		if (expectedStatusCode == SpogConstants.REQUEST_CONFLICT) {
			spogcloudRPSServer.destroyCloudHybridStoreById(ti.csr_token, hybrdstore_id, SpogConstants.SUCCESS_GET_PUT_DELETE, test);
		}
	}
