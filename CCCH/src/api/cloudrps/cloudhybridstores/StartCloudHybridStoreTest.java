...
/** This is to start the Cloud hybrid destination that is in stopped state
 * 
 * On calling the api POST: /cloudhybridstores/{id}/start Cloud hybrid store will be started if it is in stopped state
 *    
 * Prerequisites:
 ...
 * @sprint 28
 */
public class StartCloudHybridStoreTest extends base.prepare.Is4Org {
	...
	public Object[][] startCHSInfo(){
		return new Object[][] {
			...
			{"Start cloud hybrid store with msp user token",
			...
			{"Start cloud hybrid store with sub org user token",
			...
			{"Start cloud hybrid store with msp account admin token",
			...
			{"Start cloud hybrid store with sub msp user token",
			...
			{"Start cloud hybrid store with sub msp sub org user token",
			...
			{"Start cloud hybrid store with csr read only user token",
						ti.csr_readonly_token, hybridStore_id, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			
			//404
			{"Start cloud hybrid store with hybridstore id that does not exist",
						ti.csr_token, UUID.randomUUID().toString(), SpogConstants.RESOURCE_NOT_EXIST, SpogMessageCode.DESTINATION_NOT_FOUND},
			
			//405
			{"Start cloud hybrid store that is already in running state",
							ti.csr_token, hybridStore_id, SpogConstants.REQUEST_CONFLICT, SpogMessageCode.DATASTORE_IS_ALREADY_RUNNING},
			
			//200
			{"Start cloud hybrid store with valid hybridstore id",
						ti.csr_token, hybridStore_id, SpogConstants.SUCCESS_GET_PUT_DELETE, null},
		};
	}
	
	@Test(dataProvider="startCHSInfo",dependsOnMethods= {"createCloudHybridStores"})
	public void startCloudHybridStore(String caseType,
										String token, 
		...
		if (expectedStatusCode == SpogConstants.SUCCESS_GET_PUT_DELETE) {
			spogcloudRPSServer.stopCloudHybridStoreById(token, hybrdstore_id, expectedStatusCode, test);
			
			waitForSeconds(4);
		}
		
		test.log(LogStatus.INFO, caseType);
		spogcloudRPSServer.startCloudHybridStoreByIdWithCheck(token, hybrdstore_id, expectedStatusCode, expectedErrorMessage, test);
