...
/** This is to update the Cloud hybrid destination information
 * 
 * On calling the api PUT: /cloudhybridstores/{id} Cloud hybrid store information will be updated as per request payload
 *    
 * Prerequisites:
 ...
 * @sprint 28
 */
public class UpdateCloudHybridStoreByIdTest extends base.prepare.Is4Org {
	...
	HashMap<String, Object> hybridStoreInfo = new HashMap<>();
	...
	public void createCloudHybridStores(String caseType,
		...
		hybridStoreInfo = spogcloudRPSServer.composeCloudHybridStoreInfo(validToken, destination_name, rps_server_id, organization_id, volume, datacenter_id, capacity); 
	...
	@DataProvider(name="updateCloudHybridStoreInfo")
	public Object[][] updateCloudHybridStoreInfo(){
		return new Object[][] {
			//200
			{"Update cloud hybrid store with valid id", ti.csr_token, hybridStore_id,spogServer.ReturnRandom("des"),"0.01", SpogConstants.SUCCESS_GET_PUT_DELETE, null},
			
			//400
			{"Update cloud hybrid store with invalid id", ti.csr_token, "invalidId", spogServer.ReturnRandom("des"),"0.02", SpogConstants.REQUIRED_INFO_NOT_EXIST, SpogMessageCode.ELEMENT_IS_NOT_UUID},
			
			//401
			{"Update cloud hybrid store with invalid token", "invalidToken", UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT},
			{"Update cloud hybrid store with missing token", "", UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTHENTICATION_FAILED},
			{"Update cloud hybrid store with null as token", null, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTHENTICATION_FAILED},
			
			//403
			{"Update cloud hybrid store with direct user token", ti.direct_org1_user1_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Update cloud hybrid store with msp user token", ti.root_msp_org1_user1_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Update cloud hybrid store with suborg user token", ti.root_msp1_suborg1_user1_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Update cloud hybrid store with msp account admin token", ti.root_msp_org1_msp_accountadmin1_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Update cloud hybrid store with sub msp user token", ti.root_msp1_submsp1_user1_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Update cloud hybrid store with sub msp suborg user token", ti.msp1_submsp1_suborg1_user1_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Update cloud hybrid store with sub msp account admin token", ti.root_msp1_submsp1_account_admin_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"Update cloud hybrid store with csr read_only token", ti.csr_readonly_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			
			//404
			{"Update cloud hybrid store with hybridstore id that does not exist", ti.csr_token, UUID.randomUUID().toString(), spogServer.ReturnRandom("des"),"0.02", SpogConstants.RESOURCE_NOT_EXIST, SpogMessageCode.DESTINATION_NOT_FOUND},
			
		};
	}
	
	
	//add cases to update multiple destinations with same name
	
	@Test(dataProvider="updateCloudHybridStoreInfo", dependsOnMethods= {"createCloudHybridStores"})
	public void updateCloudHybridStore(String caseType,
										String token,
										String hybridStoreId,
										String hybridstore_name,
										String capacity,
										int expectedStatusCode,
										SpogMessageCode expectedErrorMessage
										) {
		...
		updateInfo.put("destination_name", hybridstore_name);
		updateInfo.put("capacity", capacity);
		
		
		HashMap<String, Object> expectedInfo = hybridStoreInfo;
		
		String IDs[] = hybridStoreId.split(",");
		for (int i = 0; i < IDs.length; i++) {
			hybridStoreId = IDs[i];
			spogcloudRPSServer.updateCloudHybridStoreByIdWithCheck(token, hybridStoreId, expectedInfo, expectedStatusCode, expectedErrorMessage, test);	
		}		
	}
