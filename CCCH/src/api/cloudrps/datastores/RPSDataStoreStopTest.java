...
/** This is to test whether the user can stop the datastore successfully or not
 ...
 *  We call the api using datastore id which we get on creation of datastore
 *  
 * Prerequisites:
...
public class RPSDataStoreStopTest extends base.prepare.Is4Org {
	...
	public void postcloudRPSDatastoreAndStop(String organizationType,
									   String organization_id,
									   String validToken,
	...
	@DataProvider(name="StopDatastoreInvalid")
	public Object stopDatastoreInvalid() {
		return new Object[][] {
			//400 cases
			{"Stopping the already STOPPED datastore", ti.csr_token, datastoreId, SpogConstants.REQUEST_CONFLICT,SpogMessageCode.DATASTORE_IS_ALREADY_STOPPED },
			{"POST datastore Stopt with ti.csr_token and invalid datastore_id.", ti.csr_token, "invalid", SpogConstants.REQUIRED_INFO_NOT_EXIST, SpogMessageCode.ELEMENT_IS_NOT_UUID},
			
			//404 cases
			{"POST datastore Stop with ti.csr_token datastore_id that does not exist.",ti.csr_token,UUID.randomUUID().toString(), SpogConstants.RESOURCE_NOT_EXIST, SpogMessageCode.DATASTORE_WITH_ID_DOESNOT_EXIST },
			
			//403
			{"POST datastore Stop with direct org user token.",ti.direct_org1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore Stop with msp org user token.",ti.root_msp_org1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore Stop valid sub org user token.",ti.root_msp1_suborg1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore Stop valid with msp_account_admin token.",ti.root_msp_org1_msp_accountadmin1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore Stop with sub msp org user token.",ti.root_msp1_submsp1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore Stop valid sub msp sub org user token.",ti.msp1_submsp1_suborg1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore Stop valid with sub msp_account_admin token.",ti.root_msp1_submsp1_account_admin_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore Stop valid with csr_readonly_token .",ti.csr_readonly_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			
			{"POST datastore Stop valid with invalid token.", "jkldfjl", datastoreId, SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT},
			{"POST datastore Stop valid with missing token.", "",datastoreId, SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTHENTICATION_FAILED }
		};
	}
	
	@Test(dataProvider= "StopDatastoreInvalid")
	public void postcloudRPSStopInvalid(String caseType, String token, String datastore_id, int expectedStatusCode, SpogMessageCode expectedErrorMessage) {

		test.log(LogStatus.INFO, caseType);
		spogcloudRPSServer.postDatastoreStopWithCheck(token, datastore_id,expectedStatusCode , expectedErrorMessage, test);
	}
