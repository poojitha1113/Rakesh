...
/** This is to test whether the user can start the datastore successfully or not
...
public class RPSDataStoreStartTest extends base.prepare.Is4Org {
	...
	public final Object[][] postcloudRPSDatastore() {
		return new Object[][] {
			{"csr",ti.csr_org_id,ti.csr_token,spogServer.ReturnRandom("RakDatastore"),true,false,false, cloudRPS_id,rps_server_name, 4,2048,false,spogServer.ReturnRandom("C:\\destination\\data"),"administrator",ti.common_password,
	...
	public void postcloudRPSDatastoreAndStart(String organizationType,
									   String organization_id,
									   String validToken,
		...
		test.log(LogStatus.INFO, "POST datastore Stop valid case with ti.csr_token.");
		spogcloudRPSServer.postDatastoreStopWithCheck(validToken, datastore_id, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);
		
		waitForSeconds(5);
		
		// 200 Cases
		test.log(LogStatus.INFO, "POST datastore start valid case with ti.csr_token.");
		spogcloudRPSServer.postDatastoreStartWithCheck(validToken, datastore_id, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);
		
	}
	
	@DataProvider(name="StartDatastoreInvalid")
	public Object StartDatastoreInvalid() {
		return new Object[][] {
			//400 cases
			{"Starting the already running datastore", ti.csr_token, datastoreId, SpogConstants.REQUEST_CONFLICT,SpogMessageCode.DATASTORE_IS_ALREADY_RUNNING },
			{"POST datastore start with ti.csr_token and invalid datastore_id.", ti.csr_token, "invalid", SpogConstants.REQUIRED_INFO_NOT_EXIST, SpogMessageCode.ELEMENT_IS_NOT_UUID},
			
			//404 cases
			{"POST datastore start with ti.csr_token datastore_id that does not exist.",ti.csr_token,UUID.randomUUID().toString(), SpogConstants.RESOURCE_NOT_EXIST, SpogMessageCode.DATASTORE_WITH_ID_DOESNOT_EXIST },
			
			//403
			{"POST datastore start with direct org user token.",ti.direct_org1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore start with msp org user token.",ti.root_msp_org1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore start valid sub org user token.",ti.root_msp1_suborg1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore start valid with msp_account_admin token.",ti.root_msp_org1_msp_accountadmin1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore start with sub msp org user token.",ti.root_msp1_submsp1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore start valid sub msp sub org user token.",ti.msp1_submsp1_suborg1_user1_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore start valid with sub msp_account_admin token.",ti.root_msp1_submsp1_account_admin_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			{"POST datastore start valid with csr readonly user token.",ti.csr_readonly_token, datastoreId, SpogConstants.INSUFFICIENT_PERMISSIONS, SpogMessageCode.RESOURCE_PERMISSION_DENY},
			
			//401
			{"POST datastore start valid with invalid token.", "jkldfjl", datastoreId, SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT},
			{"POST datastore start valid with missing token.", "",datastoreId, SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTHENTICATION_FAILED }
		};
	}
	
	@Test(dataProvider= "StartDatastoreInvalid")
	public void postcloudRPSStartInvalid(String caseType, String token, String datastore_id, int expectedStatusCode, SpogMessageCode expectedErrorMessage) {

		test.log(LogStatus.INFO, caseType);
		spogcloudRPSServer.postDatastoreStartWithCheck(token, datastore_id,expectedStatusCode , expectedErrorMessage, test);
	}
