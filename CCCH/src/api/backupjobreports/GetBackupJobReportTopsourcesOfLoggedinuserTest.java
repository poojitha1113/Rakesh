...
public class GetBackupJobReportTopsourcesOfLoggedinuserTest extends base.prepare.Is4Org {
	...
	public void beforeClass(String baseURI, String port, String adminUserName, String adminPassword, String logFolder,String runningMachine,String buildVersion) {
		...
		rep = ExtentManager.getInstance("GetBackupJobReportTopsourcesOfLoggedinuserTest", logFolder);
	...
	public void GetBackupJobReportTopsourcesOfLoggedinuser_valid(String organization_type,String token_Destination_creation,String adminToken,String organization_id,String create_user_id,
		...
		for(int i=0;i<1;i++) {
			...
			for (int k=0;k<job_type.length;k++) {
				...
				else if(job_status[k].equals("missed")||job_status[k].equals("failed")||job_status[k].equals("license_failed")||job_status[k].equals("crash"))
				{
					job_status[k]="failed";
					failed++;
				}
				...
				if(k<10)
				{
					if(!(job_status[k].equals("active")||job_status[k].equals("skipped")||job_status[k].equals("stop")||job_status[k].equals("idle")||job_status[k].equals("waiting")))
					{
						...
						temp=composeTopSources(sourceName+i,job_status[k],count);
		...
		test.log(LogStatus.INFO, "getBackupJobReportsTopSourcesForLoggedInUser");

		spogreportServer.getBackupJobReportsTopSourcesForLoggedInUser(adminToken,additionalUrl,expectedsources,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);


		test.log(LogStatus.INFO, "getBackupJobReportsTopSourcesForLoggedInUser filter with source name"+"source_name="+soucres.get(0));

		additionalUrl="source_name="+soucres.get(0);
		...
		//get backup job reports top sources, filter with random report_id
		report_id=UUID.randomUUID().toString();
		additionalUrl="report_id="+report_id;

		spogreportServer.getBackupJobReportsTopSourcesForLoggedInUser(adminToken,additionalUrl,expectedsources,SpogConstants.RESOURCE_NOT_EXIST,SpogMessageCode.UNABLE_TO_FIND_REPORT,test);

		String token = null;
		if (organization_type.contains("direct"))
			token = ti.direct_org1_monitor_user1_token;
		else if (organization_type.contains("normal_sub"))
			token = ti.normal_msp1_suborg1_monitor_user1_token;
		else if (organization_type.contains("root_sub"))
			token = ti.root_msp1_suborg1_monitor_user1_token;
		else
			token = ti.msp1_submsp1_suborg1_monitor_user1_token;
		
		spogreportServer.getBackupJobReportsTopSourcesForLoggedInUser(token,"",expectedsources,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
	...
	public void GetBackupJobReportTopsourcesOfLoggedinuser_valid_invalid(String testcase) {

		//missed token
		String additionalURL="";
		ArrayList<HashMap<String, Object>> expectedsources=new ArrayList<HashMap<String, Object>>();
		spogreportServer.getBackupJobReportsTopSourcesForLoggedInUser("",additionalURL,expectedsources,SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTHENTICATION_FAILED,test);
		//in valid token
		spogreportServer.getBackupJobReportsTopSourcesForLoggedInUser("JUNK",additionalURL,expectedsources,SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT,test);
	}
