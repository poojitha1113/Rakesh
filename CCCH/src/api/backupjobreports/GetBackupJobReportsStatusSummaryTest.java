...
public class GetBackupJobReportsStatusSummaryTest extends base.prepare.Is4Org {
	...
	public void GetBackupJobReportsStatusSummary(String organization_type,
			String token_Destination_creation,
			String adminToken,
			...
			String protected_data_size, 
			String raw_data_size, 
			String sync_read_size,
			String ntfs_volume_size,
		...
		String protectionStatus="protect,protect,protect,protect,protect,protect,protect,protect,protect,protect,protect,protect,protect,protect,protect";
		...
		for(int i=0;i<1;i++) {
			...
			for (int k=0;k<1;k++) {
				...
				job_id = gatewayServer.postJobWithCheck(start_time_ts,endTimeTS, organization_id, source_id, source_id, rps_id,destination_id, policy_id,
						job_type[k], job_method1[k], job_status[k], adminToken, test);	
				spogDestinationServer.setToken(adminToken);
				...
				spogreportServer.getBackupJobReportsDetailsStatusSummary(adminToken,additionalURL,ExpectedReports,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);

				additionalURL="source_name="+soucres.get(k);
				test.log(LogStatus.INFO,"Apply filter with "+"source_name="+soucres.get(k) );
				...
				//get backup job reports details Status Summary filter with report_id
				...
				spogreportServer.getBackupJobReportsDetailsStatusSummary(token,additionalUrl,ExpectedReports,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);

			}
		}
	}

	@DataProvider(name="GetBackupJobReportsStatusSummary_401")
	public final Object[][] getDestination_invalid(){
		return new Object[][]{
			{"invalid token to get the backupjob reports status summary"}, 
			{"invalid token to get the backupjob reports status summary"}, 
	...
	@Test(dataProvider="GetBackupJobReportsStatusSummary_401")    
	public void getbackupjobreportsforloggedinuser_invalidtoken(String testcase
		...
		String additionalURL="";
		ArrayList<HashMap<String, Object>> ExpectedReports=new ArrayList<HashMap<String, Object>>();
		spogreportServer.getBackupJobReportsDetailsStatusSummary("",additionalURL,ExpectedReports,SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTHENTICATION_FAILED,test);
		//in valid token
		spogreportServer.getBackupJobReportsDetailsStatusSummary("JUNK",additionalURL,ExpectedReports,SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT,test);
	}
