...
public class GetRestoreJobReportsDetailsTest extends base.prepare.Is4Org {
	...
	public void beforeClass(String baseURI, String port, String adminUserName, String adminPassword, String logFolder,String runningMachine,String buildVersion) {
		...
		rep = ExtentManager.getInstance("GetRestoreJobReportsDetailsTest", logFolder);
...
*/
	...
	public void GetRestoreJobReportsDetails(String organization_type,String token_Destination_creation,String adminToken,String organization_id,String create_user_id,
		...
		for(int i=0;i<1;i++) {
			...
			ArrayList<HashMap<String,Object>> schedules =policy4SpogServer.createPolicyScheduleDTO(null,schedule_id, 
					"1d", task_id, destination_id, scheduleSettingDTO, "06:00", "12:00", "cloud_direct_file_folder_backup",destination_name,test);
			...
			ArrayList<HashMap<String,Object>>  destinations= policy4SpogServer.createPolicyTaskDTO(null, task_id, "cloud_direct_file_folder_backup", destination_id, "none",null, cloudDirectFileBackupTaskInfoDTO, null, test);
			ArrayList<HashMap<String,Object>> throttles =policy4SpogServer.createPolicyThrottleDTO(null, throttle_id, task_id, "network", "1200", "1", "06:00", "18:00","cloud_direct_file_folder_backup",destination_id,destination_name, test);
			...
			String policy_name=spogServer.ReturnRandom("malleswari");
			...
			for (int k=0;k<job_type.length;k++) {
				...
				spogreportServer.setToken(adminToken);
				jobs.add(job_id);
				temp=spogreportServer.composeExpectedReports(job_id,job_status[k],source_id,sourceName+i,destination_id,destination_name,policy_id,policy_name,group_id,group_name,organization_id,organization_type);
			...
			for (int j=0;j<4;j++) {
				...
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				...
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_1,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				...
				String url_2=spogServer.PrepareURL("destination_id;=;"+destination_id, "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_2,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","destination_id;=;"+destination_id, ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);


				//filter on the organization_id
				String url_3=spogServer.PrepareURL("organization_id;=;"+organization_id, "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_3,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				...
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_4,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				...
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_5,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				...
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_6,SpogConstants.SUCCESS_GET_PUT_DELETE, null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","organization_id;=;"+UUID.randomUUID().toString(), ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				//filter on the random destination_id
				String url_7=spogServer.PrepareURL("destination_id;=;"+UUID.randomUUID().toString(), "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_7,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","destination_id;=;"+UUID.randomUUID().toString(), ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);


				//invalid filtering, given id's are not uuid

				//filter on the random policy id
				String url_8=spogServer.PrepareURL("policy_id;=;"+"uhdeided", "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_8,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","policy_id;=;"+"uhdeided", ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				spogreportServer.setToken(adminToken);
				//filter on the random source_group_id
				String url_9=spogServer.PrepareURL("source_group_id;=;"+"uhdeided", "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_9,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","source_group_id;=;"+"uhdeided", ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				spogreportServer.setToken(adminToken);

				//filter on the random organziation_id
				String url_10=spogServer.PrepareURL("organization_id;=;"+"uhdeided", "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_10,SpogConstants.SUCCESS_GET_PUT_DELETE, null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","organization_id;=;"+"uhdeided", ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				//filter on the random destination_id
				String url_11=spogServer.PrepareURL("destination_id;=;"+"uhdeided", "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_11,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","destination_id;=;"+"uhdeided", ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				//Delete policy id 
				policy4SpogServer.setToken(adminToken);
				test.log(LogStatus.INFO, "Delete the policy by policy id");
				policy4SpogServer.deletePolicybyPolicyId(adminToken,policy_id , SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				//apply filer on the deleted policy_id 
				spogreportServer.setToken(adminToken);
				String url_12=spogServer.PrepareURL("policy_id;=;"+policy_id, "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_12,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","policy_id;=;"+policy_id, ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				//deleted soucre_group_id
				spogServer.setToken(adminToken);
				spogServer.deleteGroup(group_id,test);

				spogreportServer.setToken(adminToken);
				//filter on the  deleted source_group_id
				String url_13=spogServer.PrepareURL("source_group_id;=;"+group_id, "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_13,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","source_group_id;=;"+group_id, ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);
				...
				//get backup job reports top sources, filter with report_id
				String additionalUrl="report_id;=;"+report_id;

				response=spogreportServer.getRestoreJobReportsDetails(adminToken,additionalUrl,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				...
				/*//Report_id is not uuid
				additionalUrl="report_id;=;"+123;
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,additionalUrl,SpogConstants.REQUIRED_INFO_NOT_EXIST,SpogMessageCode.UUID_IS_INVALID,test);

				//Report_id is null
				additionalUrl="report_id;=;"+"";
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,additionalUrl,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);

				//Added aditional data with the UUID
...
*/
				...
				{
					//filter on the  deleted source_group_id
					String url_14=spogServer.PrepareURL("source_name;=;"+soucres.get(j), "", 1, 20, test);
					response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_14,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
					spogreportServer.checkJobReportDetails(response, 1, 20, "","source_name;=;"+sourceName+j, ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				}

				/*//delete destiantion_id 
				spogDestinationServer.setToken(adminToken);

				test.log(LogStatus.INFO, "Delete destination by destination id");
				spogDestinationServer.deletedestinationbydestination_Id(destination_id, token_Destination_creation, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);

				//filter on the destination_id
				spogreportServer.setToken(adminToken);
				String url_14=spogServer.PrepareURL("destination_id;=;"+destination_id, "", 1, 20, test);
				response=spogreportServer.getRestoreJobReportsDetails(adminToken,url_14,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);
				spogreportServer.checkJobReportDetails(response, 1, 20, "","destination_id;=;"+destination_id, ExpectedReports, SpogConstants.SUCCESS_GET_PUT_DELETE, null, test);
				 */
	...
	public final Object[][] getDestination_invalid(){
		return new Object[][]{
			{"invalid token to get the bakupjobtopsources"}, 
			{"invalid token to get the bakupjobtopsources"}, 
	...
	public void getbackupjobreportsforloggedinuser_invalidtoken(String testcase
		...
		//missed token
		spogreportServer.getRestoreJobReportsDetails("",time_resolution, SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTHENTICATION_FAILED,test);
		//in valid token
		spogreportServer.getRestoreJobReportsDetails("JUNK",time_resolution, SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT,test);
	}
