...
public class GetRestoreJobReportTopSourcesOfLoggedInUserTest extends base.prepare.Is4Org {
	...
	String normal_subOrg_Server_id,destiantion_id;
	...
	public void beforeClass(String baseURI, String port, String adminUserName, String adminPassword,String csrReadOnlyAdminName,String csrReadOnlyAdminPassword, String logFolder,String runningMachine,String buildVersion) throws UnknownHostException {
		...
		rep = ExtentManager.getInstance("GetRestoreJobReportTopSourcesOfLoggedInUserTest", logFolder);
	...
	@DataProvider(name="getRestorejobreportsTopSources")
	public final Object[][] getRestorejobreportsTopSources(){
		return new Object[][]{
	...
	@Test(dataProvider="getRestorejobreportsTopSources")    
	public void getRestorejobreportsTopSources_valid(String organization_type,String token_Destination_creation,String adminToken,String organization_id,String create_user_id,
		...
		spogreportServer.getRestoreJobReportsTopSourcesForLoggedInUser(adminToken,expectedsources,additionalUrl,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);


		//get restore job reports top sources for logged in user filter with source name

		test.log(LogStatus.INFO, "get restore job reports top sources for logged in user filter with source name"+soucres);
		...
		//get Restore job reports top sources, filter with report_id
		additionalUrl="report_id;=;"+report_id;
		spogreportServer.getRestoreJobReportsTopSourcesForLoggedInUser(adminToken,expectedsources, additionalUrl,SpogConstants.SUCCESS_GET_PUT_DELETE,null,test);

	/*	//get Restore job reports report id is not UUID 
		additionalUrl="report_id;=;"+123;
		spogreportServer.getRestoreJobReportsTopSourcesForLoggedInUser(adminToken,expectedsources, additionalUrl,SpogConstants.REQUIRED_INFO_NOT_EXIST,SpogMessageCode.UUID_IS_INVALID,test);

		//Get Restore job Reports when add the adidtional data with the report id
		additionalUrl="report_id;=;"+123+report_id;
		spogreportServer.getRestoreJobReportsTopSourcesForLoggedInUser(adminToken,expectedsources, additionalUrl,SpogConstants.REQUIRED_INFO_NOT_EXIST,SpogMessageCode.UUID_IS_INVALID,test);


		//Get the Restore job report when pass the Report_is is null
...
*/
	...
	public final Object[][] getDestination(){
		return new Object[][]{
			{"invalid token to get the Restorejobreports"}, 
	...
	public void getRestorejobreports_401(String testcase
			){
		String missedtoken="";
		String additionalUrl="";
		//missed token
		spogreportServer.getRestoreJobReportsTopSourcesForLoggedInUser(missedtoken,null, additionalUrl,SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTHENTICATION_FAILED,test);
		//invalid token
		String invalidtoken="junk";
		spogreportServer.getRestoreJobReportsTopSourcesForLoggedInUser(invalidtoken,null, additionalUrl,SpogConstants.NOT_LOGGED_IN,SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT,test);
	}
