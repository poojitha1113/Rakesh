...
public class GetRecoveryTargetsTest extends base.prepare.Is4Org{
...
//	private String buildVersion;
	...
	@Test(priority=0)
	public void getRecoveryTargetValidParamsTest() {
		spogServer.userLogin(initial_direct_email_full_added, password);
		String sourceID1 = spogServer.createSourceWithCheck("source_name1", SourceType.machine, SourceProduct.cloud_direct,
		...
		String sourceID3 = spogServer.createSourceWithCheck("source_name3", SourceType.machine, SourceProduct.cloud_direct,
				initial_direct_orgID, siteID, ProtectionStatus.unprotect, ConnectionStatus.online, OSMajor.mac.name(),
				"sql;exchange", "vm_name",  null, "agent_name", "os_name", "64", "1.0.0", "2.0",
				"http://upgrade", UUID.randomUUID().toString(), SpogConstants.SUCCESS_POST, "", test);
		
		String sourceID4 = spogServer.createSourceWithCheck("source_name4", SourceType.machine, SourceProduct.cloud_direct,
				initial_direct_orgID, siteID, ProtectionStatus.unprotect, ConnectionStatus.online, OSMajor.unknown.name(),
				"sql;exchange", "vm_name",  null, "agent_name", "os_name", "64", "1.0.0", "2.0",
				"http://upgrade", UUID.randomUUID().toString(), SpogConstants.SUCCESS_POST, "", test);

		List<String> windowsNodes = new ArrayList<String>(Arrays.asList(new String[] {sourceID1}));
		List<String> linuxNodes = new ArrayList<String>(Arrays.asList(new String[] {sourceID2}));
		List<String> nonWindowsNodes = new ArrayList<String>(Arrays.asList(new String[] {sourceID3, sourceID2}));
		List<String> allNodes = new ArrayList<String>(Arrays.asList(new String[] {sourceID4, sourceID3, sourceID2, sourceID1}));
		
		source4spogServer.setToken(spogServer.getJWTToken());
		Response response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_file_folder_backup", "", test);
		List<String> nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(windowsNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID2, "cloud_direct_file_folder_backup", "", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(nonWindowsNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID3, "cloud_direct_file_folder_backup", "", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(nonWindowsNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_image_backup", "", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(allNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_image_backup", "image", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(allNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_image_backup", "vhdx", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(allNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_image_backup", "vmdk", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(linuxNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_hypervisor", "image", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(allNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_hypervisor", "", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(allNodes));
		
		response = source4spogServer.getRecoveryTargets(sourceID1, "cloud_direct_hypervisor", "vhdx", test);
		nodes = response.then().extract().path("data.source_id");
		assertEquals(true, nodes.equals(allNodes));
		
		source4spogServer.getRecoveryTargetsWithErrorCheck(UUID.randomUUID().toString(), "cloud_direct_file_folder_backup", "", 404, "00100201", test);
		// 2018/06/05, it bug 844360 it hangs when uuid is incorrect.
		source4spogServer.getRecoveryTargetsWithErrorCheck("invalid_uuid", "cloud_direct_file_folder_backup", "", 400, "40000005", test);
		// need a valid resource_id in another organization;
//		source4spogServer.getRecoveryTargetsWithErrorCheck("f94086c3-2a7c-4950-912c-93f2d36a507b", "cloud_direct_file_folder_backup", "", 403, "00100101", test);
	...
	/**
	 * to Test bug 891205, 892615 
	 */
	
	@Test(priority=1)
	public void getRecoveryTargetAdvTest() {
		spogServer.userLogin(initial_direct_email_full_added, password);
		source4spogServer.setToken(spogServer.getJWTToken());
		String[] sourceIDs = new String[9];
		
		for (int i = 0; i< 9; i++) {
			sourceIDs[i] = spogServer.createSourceWithCheck("source_name_adv" + String.valueOf(i), SourceType.machine, SourceProduct.cloud_direct,
					initial_direct_orgID, siteID, ProtectionStatus.unprotect, ConnectionStatus.online, OSMajor.windows.name(),
					"sql;exchange", "vm_name", null, "agent_name", "os_name", "64", "1.0.0", "2.0",
					"http://upgrade", UUID.randomUUID().toString(), SpogConstants.SUCCESS_POST, "", test);
			}
		Response response = source4spogServer.getRecoveryTargets("source_name;=;adv8", "", 1, 20, sourceIDs[1], "cloud_direct_file_folder_backup", "", test);
		List<String> nodes = response.then().extract().path("data.source_id");
		assertEquals(1, nodes.size());
		assertEquals(sourceIDs[8], nodes.get(0));
		
		response = source4spogServer.getRecoveryTargets("source_name;=;adv", "", 2, 5, sourceIDs[1], "cloud_direct_file_folder_backup", "", test);
		nodes = response.then().extract().path("data.source_id");
		...
		source4spogServer.setToken(spogServer.getJWTToken());
		response = source4spogServer.getRecoveryTargets(sourceIDs[0], "cloud_direct_hypervisor", "vhdx", test);
