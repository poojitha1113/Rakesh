...
public class GetSourceColumnsTest extends base.prepare.Is4Org{
...
//	  private String buildVersion;
	  ...
	  @DataProvider(name = "loginUserToCheckSourceColumns")
	  public final Object[][] getloginUserToCheckSourceColumns() {
			return new Object[][] { 
				{"csr"},{"csr_readonly"},{"msp"},{"direct"},{"account"},{"rootmsp"},{"submsp"},{"rootmspaccount"},{"submspaccount"}

				};
			
	  }
	  @Test(dataProvider = "loginUserToCheckSourceColumns")
	  public void checkSourceColumnsWithDifferentUsers(String loginUserType){	 
		  ...
		  if (loginUserType.equalsIgnoreCase("csr") || loginUserType.equalsIgnoreCase("msp")|| loginUserType.equalsIgnoreCase("rootmsp")|| loginUserType.equalsIgnoreCase("submsp")|| loginUserType.equalsIgnoreCase("csr_readonly")){
			  spogServer.getSourcesColumnsWithCheck4Msp(test);
		  }else{
			  spogServer.getSourcesColumnsWithCheck(test);
		  }		  
		  test.log(LogStatus.INFO,"get source header content is right");
	  }
	  
	  @Test
	  public void notLoggedGetSourceColumns(){	 
		  test=ExtentManager.getNewTest(this.getClass().getName()+"."+Thread.currentThread() .getStackTrace()[1].getMethodName());
		  test.assignAuthor("Shan, Jing");
		  //set token as null
		  spogServer.setToken("");
		  spogServer.getSourcesColumnsWithExpectedStatusCode(SpogConstants.NOT_LOGGED_IN,ErrorCode.AUTHORIZATION_HEADER_BLANK,test);
