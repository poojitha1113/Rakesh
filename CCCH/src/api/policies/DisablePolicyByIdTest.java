...
/**
 * API - POST: /policies/{id}/disable
 * Used to disable the policy by specific id
 * After trial/license expiry this api is called internally 
 * 
 * @author Rakesh.Chalamala
 * @sprint 32
 */
public class DisablePolicyByIdTest extends base.prepare.Is4Org {
	...
	String subMSPsubOrgPolicies = null;
	...
	public Object[][] testCases(){
		return new Object[][] {
			...
			{"Disable policies in direct org with sub org user token",
			...
			{"Disable policies in direct org with msp account admin user token",
			...
			{"Disable policies in direct org with sub msp org user token",
			...
			{"Disable policies in sub org with direct user token",
			...
			{"Disable policies in sub org with other msp org2 user token",
			...
			{"Disable policies in sub org with sub msp user token",
			...
			{"Disable policies in sub msp sub org with csr readonly user token",
			...
			{"Disable policies in sub msp sub org with other sub org user token",
			...
			{"Disable policies in sub msp sub org with direct user token",
			...
			{"Disable policies in sub msp sub org with other msp org2 user token",
			...
			{"Disable policies in sub msp sub org with sub msp user token",
			...
			//404
			{"Disable policies with policy id that does not exist with direct org user token",
			...
			{"Disable policies with policy id that does not exist with csr user token",
						ti.csr_token, UUID.randomUUID().toString(), SpogConstants.RESOURCE_NOT_EXIST, SpogMessageCode.NOT_FOUND_POLICY_WITH_ID},
			{"Disable policies with policy id that does not exist with sub org user token",
			...
			{"Disable policies with policy id that does not exist with msp user token",
			...
			{"Disable policies with policy id that does not exist with msp account admin user token",
			...
			{"Disable policies with policy id that does not exist with sub msp sub org user token",
			...
			{"Disable policies with policy id that does not exist with sub msp user token",
			...
			{"Disable policies with policy id that does not exist with sub msp account admin user token",
	...
	public void disablePolicyById(String testCase, 
									String token,
									String policyId,
		...
		if (expectedStatusCode == SpogConstants.SUCCESS_GET_PUT_DELETE) {
			...
			for (int i = 0; i < policies.length; i++) {
				...
				test.log(LogStatus.INFO, "Disable the already disabled policy");
				policy4SPOGServer.disablePolicyByIdWithCheck(token, policy, expectedStatusCode, expectedErrorMessage, test);
			...
			policy4SPOGServer.disablePolicyByIdWithCheck(token, policyId, expectedStatusCode, expectedErrorMessage, test);
		}
