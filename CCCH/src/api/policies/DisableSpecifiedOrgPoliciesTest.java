...
public class DisableSpecifiedOrgPoliciesTest extends base.prepare.Is4Org {
	...
	public void disableSpecifiedOrgPolicies(String testcase, String org_type, String organization_id, String validtoken,
		...
		if (!datastores.isEmpty()) {
			datastores.stream().forEach(datastore -> {
				spogcloudRPSServer.destroyCloudHybridStoreById(ti.csr_token, datastore.get("destination_id").toString(),
						SpogConstants.SUCCESS_GET_PUT_DELETE, test);
				// deleteCloudRPSDataStore(datastore.get("datastore_id").toString(),
				// ti.csr_token,
