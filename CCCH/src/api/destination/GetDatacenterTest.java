...
public class GetDatacenterTest extends base.prepare.Is4Org {
  ...
  public Object[][] userAccount(){
		  return new Object[][]{				  
				  ...
				  {final_msp_account_admin_email,common_password}, 
				  {csr_readonly_email, csr_readonly_password},
				  // root msp related
 ...
 * @throws JSONException 
   */
//  @Test()
//zhaoguo - 18/06/22, we don't need token to get datacenter
  public void getDatacenterWithoutToken() throws JSONException {
	  	test = rep.startTest("get datacenters without token");
	  	test.assignAuthor("leiyu.wang");
	  	spogServer.errorHandle.printInfoMessageInDebugFile("/****************getDatacenterWithoutToken**************/");
		...
		test.log(LogStatus.INFO,"get datacenters without token");
		spogServer.errorHandle.printInfoMessageInDebugFile("get datacenters without token");	
		//spogDestinationServer.setToken(spogServer.getJWTToken());
		Response response=spogDestinationServer.getDestinationDatacenter(200, test);	  	
	 
  }
  /**
   * get datacenter test
   * @testcase
   * csr admin could get destination/datacenters
   * @author leiyu.wang
   * @param userName
   * @param password
 * @throws JSONException 
   */
  @Test()
  public void getDatacenterWithTheLoggedUser() throws JSONException {
	  	test = rep.startTest("get datacenters with the login csr_admin user");
	  	test.assignAuthor("leiyu.wang");
	  	spogServer.errorHandle.printInfoMessageInDebugFile("/****************getDatacenterWithTheLoggedUser**************/");
	  	
		test.log(LogStatus.INFO,"get datacenters with the login user");
		spogServer.errorHandle.printInfoMessageInDebugFile("get datacenters with the login csr_admin user");	
		spogDestinationServer.setToken(spogServer.getJWTToken());
		Response response=spogDestinationServer.getDestinationDatacenter(200, test);	  	
   ...
   * get datacenter test
   * @testcase
   * 1 direct admin could get destination/datacenters
	 2 MSP admin could get destination/datacenters	 
	 3 account admin could get destination/datacenters
	 4 MSP account admin could get destination/datacenters
   * @author leiyu.wang
   * @param userName
   * @param password
 * @throws JSONException 
  ...
  public void getDatacenter(String userName, String password) throws JSONException {
	  	test = rep.startTest("get datacenters with valid user token");
	  	test.assignAuthor("leiyu.wang");
	  	spogServer.errorHandle.printInfoMessageInDebugFile("/****************getDatacenter**************/");
		...
		test.log(LogStatus.INFO,"get datacenters and check");
		spogServer.errorHandle.printInfoMessageInDebugFile("get datacenters and check");	
		spogDestinationServer.setToken(spogServer.getJWTToken());
		Response response=spogDestinationServer.getDestinationDatacenter(200, test);	  	
   ...
   * get datacenter test
   * @testcase
   * get datacenter with invalid token
   * @author leiyu.wang
   * @param userName
   * @param password
 * @throws JSONException 
   */
  @Test()
  public void getDatacenterWithInvalidToken() throws JSONException {
	  	test = rep.startTest("get datacenters with invalid token");
	  	test.assignAuthor("leiyu.wang");
	  	spogServer.errorHandle.printInfoMessageInDebugFile("/****************getDatacenterWithInvalidToken**************/");
		...
		test.log(LogStatus.INFO,"get datacenters with invalid token");
		spogServer.errorHandle.printInfoMessageInDebugFile("get datacenters with invalid token");	
		spogDestinationServer.setToken("0000000000000000.0000000000000000000000000.000000000000");
  ...
  public void beforeClass(String baseURI, String port, String logFolder, String adminUserName, String adminPassword ,  String buildVersion) throws UnknownHostException {
	  	...
	  	rep = ExtentManager.getInstance("GetDatacenterTest",logFolder);
