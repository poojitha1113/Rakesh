...
public class DeleteLoggedInUserPolicyColumnsTest extends base.prepare.PrepareOrgInfo{
	
	@Parameters({ "pmfKey"})
	public DeleteLoggedInUserPolicyColumnsTest(String pmfKey) {
		super(pmfKey);
...
private String  org_model_prefix=this.getClass().getSimpleName();
  ...
  @DataProvider(name = "organizationAndUserInfo")
	public final Object[][] getOrganizationAndUserInfo() {
		return new Object[][] {
			...
			{account_user_email,  common_password, "Name;true;1,Protected Sources;false;2"},
  ...
  /**
   * 	4. msp account admin could delete its own policy column
		3. account admin could delete   its own policy columns
		2. msp admin could delete  its own policy columns
		1. direct admin could delete its own policy columns
		5. delete policy column when not create policy column before, it should success
  ...
  public void deleteLoggedInUserPolicyColumns(String createUserPolicyColumnAdmin, String password, String createdPolicyColumns){
  ...
  @Test(enabled=true)
  /**
   * 7. fail to delete user's policy column with invalid token, report 401
	 6. fail to delete user's policy column without login, report 401

   * 
   
   */
  public void deleteLoggedInUserPolicyColumnsWithoutLogin(){
	  ...
	  test.log(LogStatus.INFO,"deleteLoggedInUserPolicyColumnsWithoutLogin");
	  policy4SpogServer.deleteLoggedInUserPolicyColumnsWithoutLoginAndCheck();
	  
	  test.log(LogStatus.INFO,"deleteUsersSourcesColumns With invalid token");
	  policy4SpogServer.deleteLoggedInUserPolicyColumnsWithCheck( UUID.randomUUID().toString(), SpogConstants.NOT_LOGGED_IN,  ErrorCode.AUTHORIZATION_HEADER_BLANK);
