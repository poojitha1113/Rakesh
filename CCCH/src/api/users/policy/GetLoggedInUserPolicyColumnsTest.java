...
public class GetLoggedInUserPolicyColumnsTest extends base.prepare.PrepareOrgInfo{
	
	@Parameters({ "pmfKey"})
	public GetLoggedInUserPolicyColumnsTest(String pmfKey) {
		super(pmfKey);
	  ...
	  @DataProvider(name = "organizationAndUserInfo")
			public final Object[][] getOrganizationAndUserInfo() {
				return new Object[][] {
					 ...
					 {final_msp_user_name_email,  common_password,"Name;true;1,Protected Sources;false;2"},					
					 {account_user_email,  common_password, "Name;true;2,Protected Sources;false;1"},
					 {final_msp_account_admin_email,  common_password, "Name;true;1,Protected Sources;false;2"},
					 {csrReadOnlyAdminUserName, csrReadOnlyAdminPassword,  "Name;true;1,Protected Sources;false;2"},
		...
		/**
			4. msp account admin could get own policy columns
			3. account admin could get   its own policy columns
			2. msp admin could get  its own policy columns
			1. direct admin could get  its own policy columns
		 ...
		 */
		  ...
		  public void getLoggedInUserSourcesColumns(String createUserPolicyColumnAdmin, String password, String specifiedPolicyColumns){
			  ...
			  test.log(LogStatus.INFO,"get user policy columns");
			  policy4SpogServer.getLoggedInUserPolicyColumnsWithCheck(columnsList, token, SpogConstants.SUCCESS_GET_PUT_DELETE, null);
		  ...
		  @Test(enabled=true)
		  /**
			6. fail to get user's policy column with invalid token, report 401
			5. fail to get user's policy column without login, report 401

		   */
		  public void getLoggedInUserPolicyColumnsWithoutLogin(){
			  ...
			  String token = spogServer.getJWTToken();
			  test.log(LogStatus.INFO,"getLoggedInUserPolicyColumnsWithoutLogin");
			  policy4SpogServer.getLoggedInUserPolicyColumnsWithoutLoginAndCheck();
			  
			  test.log(LogStatus.INFO,"getLoggedInUserPolicyColumnsWithCheck With invalid token");
			  policy4SpogServer.getLoggedInUserPolicyColumnsWithCheck(null, UUID.randomUUID().toString(), SpogConstants.NOT_LOGGED_IN,  ErrorCode.AUTHORIZATION_HEADER_BLANK);
