...
public class UpdateloggedInUserRecoveredResourcesFiltersTest extends base.prepare.Is4Org {
	...
	@DataProvider(name = "update_RecoveredResources_filter_valid")
	public final Object[][] updateRecoveredResourcesFilterValidParams() {
	...
	@Test(dataProvider = "update_RecoveredResources_filter_valid")
	public void updateRecoveredResourcesFilterValid_200(String organizationType, String validToken, String user_id,
		...
		test.log(LogStatus.INFO, "update the created filters in the organization");

		response = spogRecoveredResourceServer.updateLoggedInUserRecoveredResourcesFilters(validToken, filter_id,
				policy_id, State.get(3), OSmajor, recoveredResourceType.get(0), filter_name + "abc", is_default,
	...
	@DataProvider(name = "update_RecoveredResources_filter_invalid")
	public final Object[][] updateRecoveredResourcesFilterInValidParams() {
	...
	// get the user defined job columns using csr/valid token
	@Test(dataProvider = "update_RecoveredResources_filter_invalid")
	public void updateRecoveredResourcesFilterValid_401(String organizationType, String validToken, String user_id,
			String org_id, String policy_id, String state, String OSmajor, String RecoveredResourceType,
			String filter_name, String is_default) {
		...
		Response response = spogRecoveredResourceServer.updateLoggedInUserRecoveredResourcesFilters(inValidToken,
				UUID.randomUUID().toString(), policy_id, State.get(3), OSmajor, recoveredResourceType.get(0),
				filter_name + "abc", is_default, SpogConstants.NOT_LOGGED_IN, test);

		spogRecoveredResourceServer.checkRecoveredResourceFilters(response, UUID.randomUUID().toString(), policy_id,
				State.get(3), OSmajor, recoveredResourceType.get(0), filter_name + "abc", is_default, org_id, user_id,
				SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTENTICATION_FAILED_FOR_JWT, test,
				filtersHeadContent);

		String MissingToken = "";

		test.log(LogStatus.INFO, "update filters in the organization with missing token");

		response = spogRecoveredResourceServer.updateLoggedInUserRecoveredResourcesFilters(MissingToken,
				UUID.randomUUID().toString(), policy_id, State.get(3), OSmajor, recoveredResourceType.get(0),
				filter_name + "abc", is_default, SpogConstants.NOT_LOGGED_IN, test);

		spogRecoveredResourceServer.checkRecoveredResourceFilters(response, UUID.randomUUID().toString(), policy_id,
				State.get(3), OSmajor, recoveredResourceType.get(0), filter_name + "abc", is_default, org_id, user_id,
				SpogConstants.NOT_LOGGED_IN, SpogMessageCode.COMMON_AUTHENTICATION_FAILED, test, filtersHeadContent);

	}

	// get the user defined job columns using csr/valid token
	@Test(dataProvider = "update_RecoveredResources_filter_invalid")
	public void updateRecoveredResourcesFilterInValid_filter(String organizationType, String validToken, String user_id,
			String org_id, String policy_id, String state, String OSmajor, String RecoveredResourceType,
			String filter_name, String is_default) {
		...
		test.log(LogStatus.INFO, "update filters in the organization with invalid filter id");

		response = spogRecoveredResourceServer.updateLoggedInUserRecoveredResourcesFilters(validToken, "abc", policy_id,
				State.get(2), OSmajor, recoveredResourceType.get(0), filter_name + "abc", is_default,
		...
		spogRecoveredResourceServer.checkRecoveredResourceFilters(response, filter_id, policy_id, state, OSmajor,
				...
				SpogMessageCode.ELEMENT_FILTERID_IS_NOT_UUID, test, filtersHeadContent);

		test.log(LogStatus.INFO, "delete filters in the organization with randomuuid filter id");

		String invalidfilter = UUID.randomUUID().toString();

		response = spogRecoveredResourceServer.updateLoggedInUserRecoveredResourcesFilters(validToken, invalidfilter,
				policy_id, State.get(2), OSmajor, recoveredResourceType.get(0), filter_name + "abc", is_default,
				SpogConstants.RESOURCE_NOT_EXIST, test);

		spogRecoveredResourceServer.checkRecoveredResourceFilters(response, invalidfilter, policy_id, state, OSmajor,
