...
public class EnhanceUpdateSourceFilterTest extends base.prepare.Is4Org{
...
//	  private String buildVersion;
	  ...
	  @DataProvider(name = "loginUserToUpdateDifferentFilterItems")
	  public final Object[][] getLoginUserToUpdateDifferentFilterItems() {
			return new Object[][] {  
				{"account"},{"csr"},{"direct"},{"msp"},
				{"submsp"},{"rootmsp"},{"submspaccount"},
			};
	  }
	  @Test(dataProvider = "loginUserToUpdateDifferentFilterItems")
	  public void updateFilterWithDifferentFilterItems(String loginUserType){	 
		  ...
		  String pre_filter_id=spogServer.createFilterwithCheck(user_id, pre_filter_name, "none", "none", "none", "none", "none", "none", "none","none",sourceName,sourceType,"none", test);
									  ...
									  for(int operating_i=0;operating_i<operating_array.length;operating_i++){
										  if(application_array.length>0){
											  for(int application_i=0;application_i<application_array.length;application_i++){
												  ...
												  spogServer.updateFilterwithCheck(user_id, pre_filter_id,tem_filter_name, protection_array[protection_i],connect_array[connect_i],protection_policy,backup_array[backup_i], source_group, operating_array[operating_i], application_array[application_i],siteID,sourceName,sourceType,sourceType,"false", test);
											  }
										  }
									  }
								  }
	  ...
	  @DataProvider(name = "loginUserToUpdateDifferentSourceName")
	  public final Object[][] getLoginUserToUpdateDifferentSourceName() {
			return new Object[][] {{"account","right"}};
	  }
	  //@Test(dataProvider = "loginUserToUpdateDifferentSourceName")
	  public void updateFilterWithDifferentSourceName(String loginUserType,String source_name){	 	 
		  test=ExtentManager.getNewTest(this.getClass().getName()+"."+Thread.currentThread() .getStackTrace()[1].getMethodName());
		  test.assignAuthor("Shan, Jing");
		  String user_id;
		  ...
		  String tem_filter_name = spogServer.returnRandomUUID();
		  String pre_filter_id=spogServer.createFilterwithCheck(user_id, pre_filter_name, "none", "none", "none", "none", "none", "none", "none","none",sourceName,sourceType,"none", test);
		  spogServer.createFilterwithCheck(user_id, tem_filter_name, "none", "none", "none", "none", "none", "none", "none","none",sourceName,sourceType,"none", test);
		  test.log(LogStatus.INFO,loginUserType+ " admin create filter with source name is "+sourceName); 
		  if (source_name==null){
			  spogServer.updateFilterwithCheck(user_id,pre_filter_id, pre_filter_name, null, null, null, null, null, null,null,null,source_name,sourceType,sourceType,"false" , test);
			  test.log(LogStatus.INFO,loginUserType+ " admin update filter with source name is empty or null will be failed"); 
		  }else if (source_name.equalsIgnoreCase("")){
			  test.log(LogStatus.INFO,loginUserType+ " admin update filter with source name is empty or null will be failed"); 
		  }else if(source_name.equalsIgnoreCase("random")){
			  source_name=spogServer.returnRandomUUID();
			  spogServer.updateFilterwithCheck(user_id,pre_filter_id, pre_filter_name,"none", "none", "none", "none", "none", "none", "none","none",source_name,sourceType,sourceType,"false", test);
			  test.log(LogStatus.INFO,loginUserType+ " admin update filter with source name is "+source_name); 
		  }else if(source_name.equalsIgnoreCase("right")){
			  String userToken = spogServer.getJWTToken();
			  String prefix = RandomStringUtils.randomAlphanumeric(8);
			  String siteID = gatewayServer.createsite_register_login(organizationID, userToken, user_id, prefix + "enhance",
						"1.0.0", spogServer, test);
			  spogServer.updateFilterwithCheck(user_id, pre_filter_id, pre_filter_name,"none", "none", "none", "none", "none", "none", "none",siteID,source_name,sourceType,sourceType,"false", test);
				  test.log(LogStatus.INFO,loginUserType+ " admin update filter with source name is "+sourceName); 
		  }
		  if(loginUserType.equalsIgnoreCase("csr")){
			  for(int i=0;i<no_permission_users_id.length;i++){
				  spogServer.userLogin(no_permission_users_id[i], OrgPwdForPrepare,test);
				  spogServer.updateFilterFailedWithExpectedStatusCode(user_id, pre_filter_id,pre_filter_name, "none", "none", "none", "none", "none", "none", "none", SpogConstants.INSUFFICIENT_PERMISSIONS,no_permission_users_errorCode[i], test);
		  ...
		  if(permission_users_id!=null){
			  for(int i=0;i<permission_users_id.length;i++){
				  ...
				  spogServer.updateFilterwithCheck(user_id, pre_filter_id,pre_filter_name, "none", "none", "none", "none", "none", "none", "none", test);
	  ...
	  public void notLoggedCreateFilter(){	 
		  ...
		  spogServer.setToken("");
		  spogServer.updateFilterFailedWithExpectedStatusCode(user_id, "dd", "none", "none", "none", "none", "none", "none", "none",  "none", "none",  "none", SpogConstants.NOT_LOGGED_IN, ErrorCode.AUTHORIZATION_HEADER_BLANK,test);
	  }
	  ...
	  public void csrReadOnlyNotCreateFilter(){	 
		  ...
		  String user_id=spogServer.GetLoggedinUser_UserID();
		  String source_name=spogServer.ReturnRandom("dd");
		  String update_source_name=spogServer.ReturnRandom("dd");
		  String filter_id=spogServer.createFilterwithCheck(user_id, source_name, "none", "none", "none", "none", "none", "none", "none","none", "none", test);
		  spogServer.updateFilterwithCheck(user_id, filter_id, update_source_name,"none", "none", "none", "none", "none", "none", "none",  "none", "none",  "none", test);
	  }
