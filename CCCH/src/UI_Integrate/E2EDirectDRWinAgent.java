...
public class E2EDirectDRWinAgent extends base.prepare.Is4Org {
	...
	private String restore_file="X:\\02\\restore_dest\\test.txt";
	...
	private String dr_source="spog-ui-d-w16";
	...
	public void beforeClass(String baseURI, String port, String browserType, String maxWaitTimeSec, String logFolder,
		...
		Sources="spog-ui-d-w16";
		String Sourcename="spog-ui-d-w16";
...
//		baas_destination_volume = response.then().extract().path("data[" + 0 + "].destination_name").toString();
	...
	public final Object[][] getOrganizationAndUserInfo1() {
		return new Object[][] {
				...
				{  this.direct_org_email, common_password, "baas image test" + RandomStringUtils.randomAlphanumeric(8),
					null, dr_source, UIConstants.ACTIVE_OPTION_SELECTDRIVE, "C", draas_destination_volume,
					"false", null, UIConstants.BACKUP_EVERY_1_DAY, "08", "55", null, null,
	...
	public void addDraasPolicy(String userName, String password, String policyName, String description,
		...
		String tmp_source_file="jdk-8u92-windows-x64"+ RandomStringUtils.randomAlphanumeric(8)+".exe";
		//prepare backup source file
		try {
			folderOrFileUtils.mountNetDrive("\\\\10.55.16.200\\C$", "X:", "administrator", "123.com");
			folderOrFileUtils.createDirecotry("X:\\backupsource");
			folderOrFileUtils.copyFile("X:\\backupsource1\\test.txt", "X:\\backupsource\\test.txt");
			boolean fileExists=folderOrFileUtils.checkFileExist("X:\\backupsource\\\\test.txt");
		...
		String last_job_time=sourceHelper.getLastJobTimeFromLatestJobColume(dr_source, test);
		...
		sourceHelper.startBackup(dr_source, test);
		
		//wait job to in progress status in job page
		jobsHelper.waitExpectedJobInSavedSearch("Jobs in Progress", "Backup from "+ dr_source + " to "+draas_destination_volume, 200, test);		
		...
		monitorHelper.checkInProgessJobInformation("Backup from " + dr_source + " to "+draas_destination_volume,dr_source,test);
				
		//check the job status is expected in source 2 jobs
		//sourceHelper.checkJobinformationFromLatestJobColume("spog-ui-d-w16","backup-incremental","In progress", test);
		sourceHelper.waitUntilJobUpdatedFromLatestJobColume(dr_source,"backup",last_job_time,"In progress","2 Jobs",2, test);
		
		
		//wait job to finish
		jobsHelper.waitJobDone("Backup from "+ dr_source + " to "+draas_destination_volume, 5000, test);
		HashMap<String, Object> expectedBackupJobInfo = new HashMap<>();
		expectedBackupJobInfo.put(TableHeaders.source, dr_source);
		expectedBackupJobInfo.put(TableHeaders.destination, draas_destination_volume);
		expectedBackupJobInfo.put(TableHeaders.job_name, "Backup from "+ dr_source + " to "+draas_destination_volume);
		...
		jobsHelper.checkEssentialJobInformation("Backup from "+ dr_source + " to "+draas_destination_volume, expectedBackupJobInfo, test);
		//check job status is expected in source 2 jobs
		//sourceHelper.checkJobinformationFromLatestJobColume("spog-ui-d-w16","backup-incremental","Finish", test);
		sourceHelper.waitUntilJobUpdatedFromLatestJobColume(dr_source,"backup",last_job_time,"Finished","2 Jobs",2, test);
		
		//wait job to in progress status
		jobsHelper.clickViewLogsForGivenJob("Backup from "+ dr_source + " to "+draas_destination_volume, test);
		HashMap<String, Object> expectedFirstLogInfo = new HashMap<>();
		expectedFirstLogInfo.put(TableHeaders.source, dr_source);
		expectedFirstLogInfo.put(TableHeaders.generated_from, dr_source);
		expectedFirstLogInfo.put(TableHeaders.job_name, "Backup from "+ dr_source + " to "+draas_destination_volume);
		...
		expectedLastLogInfo.put(TableHeaders.source, dr_source);
		expectedLastLogInfo.put(TableHeaders.generated_from, dr_source);
		expectedLastLogInfo.put(TableHeaders.job_name, "Backup from "+ dr_source + " to "+draas_destination_volume);
		...
		try {
			folderOrFileUtils.deleteFile("X:\\02\\restore_dest\\test.txt");
		}catch(Exception e) {
			e.printStackTrace();
		}
		String previousStartTime=jobsHelper.getLastColumeValueFromSuccessfulJobs(TableHeaders.start_time,dr_source, test);
		System.out.println("currentStartTime is:"+previousStartTime);
		source4spogServer.setToken(spogServer.getJWTToken());
		String restore_job_id=source4spogServer.submitCDRestoreJob("91f97416-8f19-42a9-bd3f-96b720c1e10d", "91f97416-8f19-42a9-bd3f-96b720c1e10d", "/ZettaMirror/zsystem3/VOLUME/C.img-expand/02/backupsource/", "C:\\02\\restore_dest\\", null, true, false, "cloud_direct_draas",test);
		
//		sourceHelper.startRecover(dr_source, test);
//		test.log(LogStatus.INFO, "start recover job");
//		jobsHelper.clickRecoveryFromFristRow(test);
//		jobsHelper.recoveryToOriginal("c:\\source",test);
		//wait job to finish
		jobsHelper.waitJobDone("Restore from "+ dr_source + " to "+ dr_source,"Restore",previousStartTime, 30, test);		
		...
		expectedBackupJobInfo.put(TableHeaders.source, dr_source);
		expectedBackupJobInfo.put(TableHeaders.destination, draas_destination_volume);
		expectedBackupJobInfo.put(TableHeaders.job_name, "Restore from "+ dr_source + " to "+ dr_source );
		expectedBackupJobInfo.put(TableHeaders.job_type, "Restore");
		expectedBackupJobInfo.put(TableHeaders.status, "Finished");
		expectedBackupJobInfo.put(TableHeaders.policy, policyName);
		expectedBackupJobInfo.put(TableHeaders.start_time, currentStartTime);
		//check the essential job information from all jobs
		jobsHelper.checkEssentialJobInformation("Restore from "+ dr_source + " to "+ dr_source , expectedBackupJobInfo, test);
		//check job status is expected in source 2 jobs
		//sourceHelper.checkJobinformationFromLatestJobColume("spog-ui-d-w16","backup-incremental","Finish", test);
		sourceHelper.waitUntilJobUpdatedFromLatestJobColume(dr_source,"Restore",null,"Finished","2 Jobs",2, test);
		
		//wait job to in progress status
		jobsHelper.clickViewLogsForGivenJob("Restore from "+ dr_source + " to "+ dr_source , test);
		expectedFirstLogInfo = new HashMap<>();
		expectedFirstLogInfo.put(TableHeaders.source, dr_source);
		expectedFirstLogInfo.put(TableHeaders.generated_from, dr_source);
		expectedFirstLogInfo.put(TableHeaders.job_name, "Restore from "+ dr_source + " to "+ dr_source );
...
//		try {
//			folderOrFileUtils.deleteFile("Y:\\backupsource\\"+tmp_source_file);
//		}catch(Exception e) {
//			e.printStackTrace();
//		}
		folderOrFileUtils.deleteDirectory(new File("X:\\backupsource"));
	}
