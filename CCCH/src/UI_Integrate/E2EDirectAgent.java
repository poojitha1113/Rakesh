...
public class E2EDirectAgent extends base.prepare.Is4Org {
	...
	private String restore_file="X:\\backupsource\\test.txt";
...
//		baas_destination_volume = response.then().extract().path("data[" + 0 + "].destination_name").toString();
	...
	public void UITestForDirectUser(String userName, String password, String policyName, String description,
		...
		try {
			folderOrFileUtils.mountNetDrive("\\\\10.55.16.200\\C$", "X:", "sj-win2016-2\\administrator", "123.com");
			folderOrFileUtils.createDirecotry("X:\\backupsource");
			folderOrFileUtils.copyFile("X:\\backupsource1\\jdk-8u92-windows-x64.exe", "X:\\backupsource\\"+tmp_source_file);
		...
		jobsHelper.waitExpectedJobInSavedSearch("Jobs in Progress", "Backup from "+sourceNames+" to "+baas_destination_volume, 2, test);		
		...
		previous_dest_last_time=destinationHelper.getLastJobTimeFromLatestJobColume(1, test);
		
		//delete restore source file
		try {
			folderOrFileUtils.deleteFile("X:\\backupsource\\test.txt");
		}catch(Exception e) {
			e.printStackTrace();
		}
		...
		//wait job to finish
		jobsHelper.waitJobDone("Restore from "+sourceNames+" to "+sourceNames,"Restore",previousStartTime, 30, test);		
		...
		//check job status is expected in source 2 jobs
		//sourceHelper.checkJobinformationFromLatestJobColume("spog-ui-d-w16","backup-incremental","Finish", test);
		sourceHelper.waitUntilJobUpdatedFromLatestJobColume(sourceNames,"Restore",null,"Finished","2 Jobs",2, test);
		...
		try {
			folderOrFileUtils.deleteFile("X:\\backupsource\\"+tmp_source_file);
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
