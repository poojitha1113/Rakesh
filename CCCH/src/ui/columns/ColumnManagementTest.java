...
public class ColumnManagementTest extends base.prepare.Is4Org{
	...
	public void beforeClass(String baseURI, String port, String logFolder, String adminUserName, String adminPassword,  String buildVersion,
		...
		if (uiBaseURI.contains("lccitc-fe")) {
	...
	public Object[][] columnsInfo(){
		return new Object[][] {
			{"sources", allSourceColumns, defaultSourceColumns},
			{"destinations", allDestinationColumns, defaultDestinationColumns},
			{"recovered_vms", allRecoveredResourcesColumns, defaultRecoveredResourcesColumns},
			{"policies", allPolicyColumns, defaultPolicyColumns},
			{"jobs", allJobColumns, defaultJobColumns},
			{"backup_job_reports", allBackupJobReportsColumns, defaultBackupJobReportsColumns},
			{"recovery_job_reports", allRecoveryJobReportsColumns, defaultRecoveryJobReportsColumns},
			{"users", allUsersColumns, defaultUsersColumns},
		};
	}
	
	@Test(priority=0, dataProvider="columnsInfo")
	public void deselectAllColumnsTest(String page, ArrayList<String> allColumns, ArrayList<String> defaultColumns) {
		test=ExtentManager.getNewTest(this.getClass().getName()+"."+Thread.currentThread() .getStackTrace()[1].getMethodName());
		
		columnHelper.goToPage(page);
		
		test.log(LogStatus.INFO, "Deselect all columns and verify in page:"+page);
		columnHelper.unSelectAllColumnsWithCheck(allColumns, defaultColumns, test);
	}
	
	@Test(priority=1, dataProvider="columnsInfo")
	public void enableSomeColumnsTest(String page, ArrayList<String> allColumns, ArrayList<String> defaultColumns) {
		
		test=ExtentManager.getNewTest(this.getClass().getName()+"."+Thread.currentThread() .getStackTrace()[1].getMethodName());
		
		ArrayList<String> columns = new ArrayList<>();
		int columnCount = new Random().nextInt(allColumns.size())+1;
		...
		test.log(LogStatus.INFO, "Enable some columns and verify in page:"+page);
		columnHelper.enableColumnsWithCheck(columns, test);
		
	}
	
	@Test(enabled=true, priority=2, dataProvider="columnsInfo")
	public void enableAllColumnsTest(String page, ArrayList<String> allColumns, ArrayList<String> defaultColumns) {
		
		test=ExtentManager.getNewTest(this.getClass().getName()+"."+Thread.currentThread() .getStackTrace()[1].getMethodName());
		
		columnHelper.goToPage(page);
		
		test.log(LogStatus.INFO, "Enable all columns and verify in page:"+page);
		columnHelper.enableColumnsWithCheck(allColumns, test);
